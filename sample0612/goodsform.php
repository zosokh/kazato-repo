<?php
        ...
        //カラー
        $color_order[$goods["id"]] = $_POST["color_$goods[id]"] ;

        //個数
        $qtyall = "";
        $qty_order[$goods["id"]] = $_POST["qty_$goods[id]"] ;
        $qtyall += $qty_order[$goods["id"]];
        // echo $qtyall;
            


        if ($_POST["qty_$goods[id]"] == "zero"){
                $itemformat_[$goods["id"]] = null;
            }else{
                $itemformat_[$goods["id"]] = "$goods[name]" ."\n" . "■色: ". $color_order[$goods["id"]] ."\n" . "■個数：" . $qty_order[$goods["id"]] ."個" . "\n" . "<br>";
                //全ての情報
                $order_info .= $itemformat_[$goods["id"]];
            }
        ...
        //全ての情報
        $order_info .= $phone_num_order . $adress_order . "<br>";
        ...
        // //全ての情報
        // $order_info = $phone_num_order . $adress_order ;
        // $order_info2 = allitemsFormat() ;
        // echo $order_info;
...
<?php
        ...
        function process_form(){
            ...
            foreach ($items as $goods) {
                if ($_POST["qty_$goods[id]"] !== "zero"){
                        echo "<img  style='width:100px; height:100px;' src='".$goods['img']."'>";
                    }
            }
            echo "</div> ";
